...
class Env:
    ...
    def step(self, prompts, reward_prompts, labels, temperature=0.01, top_p=0.9, memory=None):
        ...
        #---------------------------------------------------------
        add_pre = rewards[0] * 100 - memory.pre_reward  
        ret_reward = rewards[0] * 100 + memory.pre_reward
        memory.pre_reward = rewards[0] * 100

        del generated_id
        torch.cuda.empty_cache()
        return decoded, ret_reward, all_unfinished[0], rewards[0], add_pre
        #----------------------------------------------------------
    ...
    def label2inputid(self, labels):
        ...
        if template_name == 'vicuna':
            labels2ids={'yes':3869,'no':1939}
        else:
            labels2ids={'yes':8241,'no':3782}
...
#------------------------------------------------------------------------------------
data_train = load_dataset("/home/kai/alpaca-lora/data/commonsense_qa_2.0", split='train') 
data_dev = load_dataset("/home/kai/alpaca-lora/data/commonsense_qa_2.0", split='validation')
#------------------------------------------------------------------------------------
...
def cs_knowledge_collote_fn(batch_samples):
    ...
    for sample in batch_samples:            
        if template_name == 'vicuna' :
            p = prompter.generate_prompt(instruction="Answer the question based on knowledge. If the question is correct, answer 'yes', if the question is incorrect, answer 'no'. Knowledge: [replace_here] Question: "+sample['question'])
            ...
            p = prompter.generate_prompt(instruction="Answer the question based on knowledge. If the question is correct, answer 'yes', if the question is incorrect, answer 'no'.",input='Question:\n'+sample['question']+'\nKnowledge:\n[replace_here]')
    ...
    return batch_data, batch_gen_know, batch_question, batch_label
...
def main():
    jsonl_file = "vicuna_gen_csqa2_train.jsonl"
    ...
    with jsonlines.open(jsonl_file, mode='w') as writer:
        for epoch in range(max_epoch):
            ...
            for idx, (batch_data, batch_gen_know, batch_question, batch_label) in enumerate(dev_dataloader):
                temperature = 1
                ...
                while done == 1:
                    ...
                    decoded, reward, done, raw_reward, add_pre = env.step(prompts=combined_prompts, reward_prompts=batch_data, temperature=temperature, labels=batch_label, memory=memory)
                    ...
                    if preds[0][0:3].lower() == 'yes':
                        ...
                        preds[0] = 'no'
                    elif 'yes' in preds[0].lower():
                        preds[0] = 'yes'
                    elif 'no' in preds[0].lower():
                        preds[0] = 'no'
                    else:
                        break
                    
                    data_record = {'question': batch_question[0], 'knowledge': decoded[0], 'score': raw_reward.item(), 'pred': preds[0]}
