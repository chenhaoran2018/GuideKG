...
def cs_knowledge_collote_fn(batch_samples):
    ...
    for sample in batch_samples:
        ...
        if template_name == 'vicuna' :
            p = prompter.generate_prompt(instruction="Answer the question based on knowledge. Answer 'Yes' or 'No'. Knowledge: [replace_here] Question: "+sample['question'])
...
# 验证数据，看真实的回答正确率（考虑改为不让自动结束，手动设置生成句子数）
import time
import random
@torch.inference_mode()
def test():
            ...
            if sorted_list[0] != sorted_list[-1] and sorted_list[0] != state[1] and sorted_list[-1] != state[1]:  # 现在相同的时候会跳过融合
                ...
                if re_error==False :
                    if template_name=='vicuna':
                        ...
                        fusion_prompts = prompter.generate_prompt(instruction='Rewriting the given knowledge into a new sentence requires retaining the part of the given knowledge that is relevant to the question and correct.',input=inputs_know+' Question: '+batch_question[0])
