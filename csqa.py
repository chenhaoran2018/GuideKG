...
class Env:
    ...
    def guide_gen(self, prompts, reward_prompts, labels, temperature=0.9, top_p=0.9):
        self.model.generation_config.temperature = temperature
        self.model.generation_config.top_p = top_p
        generated_id, all_unfinished = self.generate(
            prompts=prompts,
...
data_dev = load_dataset(dataset_file_path, split='validation')
...
def test():
    ...
    for idx, (batch_data, batch_gen_know, batch_question, batch_choice, batch_label) in enumerate(dev_dataloader):
        ...
        while done == 1:
            ...
            rank_list = [] 
            while threshold > 0:
                decoded, reward, done = env.guide_gen(prompts=combined_prompts, reward_prompts=batch_data, labels=batch_label, temperature=1)  # temperature实验
